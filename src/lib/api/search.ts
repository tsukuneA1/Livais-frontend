...
export const searchPosts = async (query: string): Promise<Result<Post[]>> => {
	const result = await apiClient.get<Post[]>(`/api/v1/search?q=${encodeURIComponent(query)}&f=live`);
...
export const searchUsers = async (query: string): Promise<Result<User[]>> => {
	const result = await apiClient.get<User[]>(`/api/v1/search?q=${encodeURIComponent(query)}&f=users`);
